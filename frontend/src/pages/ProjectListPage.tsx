      ...
      <main style={main}>
        {showForm && (
          <form onSubmit={handleCreate} style={formCard}>
            ...
            {createError && (
              <p role="alert" style={{ color: "var(--color-danger)", fontSize: "0.875rem", margin: "0.25rem 0 0" }}>
                {createError}
              </p>
            )}
        ...
        {isError && <p style={{ color: "var(--color-danger)" }}>{t("projects.failedToLoad")}</p>}
        {data && data.length === 0 && !showForm && (
          <p style={{ color: "var(--color-text-secondary)" }}>{t("projects.noProjects")}</p>
        )}
        {data && (
    ...
    <Link to={`/projects/${p.id}`} style={plateLink}>
      <div style={plate}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "0.5rem" }}>
          <span style={{ fontWeight: 700, fontSize: "1rem", color: "var(--color-text-primary)", flex: 1, lineHeight: 1.3 }}>
            {p.name}
          </span>
          {p.code && (
...
function TicketBucket({ label, count, color }: { label: string; count: number; color: string }) {
  return (
    <div style={bucketBox}>
      ...
      <span style={{ fontSize: "0.7rem", color: "var(--color-text-secondary)", marginTop: "0.1rem" }}>{label}</span>
    </div>
  );
}
