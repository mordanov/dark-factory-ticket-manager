...
def build_html_report(aggregates: dict[str, object]) -> str:
    ...
    total_seconds = aggregates["total_seconds"]
    total_tokens = aggregates["total_tokens"]
    unknown_token_events = aggregates["unknown_token_events"]
...
<body>
  ...
  <section class=\"summary\">
    ...
    <div class=\"card\"><strong>Total time</strong><div>{format_duration(int(total_seconds))}</div></div>
...
def print_summary(aggregates: dict[str, object]) -> None:
    print(f"Events: {aggregates['total_events']}")
    print(f"Time: {format_duration(int(aggregates['total_seconds']))}")
    print(f"Tokens: {aggregates['total_tokens']}")
    print(f"Unknown token entries: {aggregates['unknown_token_events']}")
    print()
    print("By agent:")
    for name, stats in aggregates["by_agent"].items():
        print(
            f"  - {name}: {stats['events']} events, {format_duration(stats['seconds'])}, {stats['tokens']} tokens"
        )
    print()
    print("By feature:")
    for name, stats in aggregates["by_feature"].items():
        print(
            f"  - {name}: {stats['events']} events, {format_duration(stats['seconds'])}, {stats['tokens']} tokens"
        )
    print()
    print("By model:")
    for name, stats in aggregates["by_model"].items():
        print(
            f"  - {name}: {stats['events']} events, {format_duration(stats['seconds'])}, {stats['tokens']} tokens"
        )
